$(function(){
	...
	var updateFilelist = function(){
		...
		// List stored
		//var connection = new WebSocket("ws://10.0.0.100:81",'json');
		var connection = new WebSocket("ws://10.0.0.100:81"); // Im pretty sure the server is not parsing json...
		connection.onopen = function () { console.loc("Connecting to Gumstix...");connection.send("U"); };
		...
		connection.onerror = function (error) { alert('Error: websocket connection failed: ' + error);console.log(error) };
	};
	updateFilelist();
